%======TO RUN THE PROGRAM CALL THE PREDICATE solution.======%

% according to the concept of a* search, we need to store open and closed points in mainly 2 lists
:- dynamic open/6.
:- dynamic closed/6.
...
:- dynamic numOfSteps/1.

% everyting on the map is an object, and making sure that they do not interract
...
% defining an illegal place to put an object in
        ...
        (   
            ...
            forall(open(X0, Y0, G0, H0, PrevX0, PrevY0),
                (   
                    ...
                    (   H0 + G0 =< H1 + G1, \+pointlessToGo(PrevX0, PrevY0, X0, Y0)) -> ( 
...
check(X, Y, PrevX, PrevY, M):-
    ...
    (
        ...
        (   doctor(DoctorX, DoctorY) ; true),
        (   (   MaskX == X, MaskY == Y); (   DoctorX == X, DoctorY == Y)) -> 
        (
            assert(open(X, Y, G, H, PrevX, PrevY)),   
...
% To prevent from going towards the covid
pointlessToGo(X, Y, NextX, NextY):-
    home(HomeX, HomeY),
    (   NextX \= HomeX; NextY \= HomeY),
    covid(CovidX, CovidY),
    home(HomeX, HomeY),
    abs(CovidX - X, DX),
    abs(CovidY - Y, DY),
    (   
        (  
            DX == 3,
            DY =< 1,
            NextX is X + (CovidX - X)/3,
            NextY is Y
        );
        (   
            DY == 3,
            DX =< 1,
            NextY is Y + (CovidY - Y)/3,
        	NextX is X
        )
    ).
...
incrementSteps:-
    ...
    NumOfSteps0 is NumOfSteps + 1,
    retractall(numOfSteps(_)),
    assert(numOfSteps(NumOfSteps0)).

% the main function
% spawning everything
% preparing all the dynamic predicates to work with later
% having an outcome for the kotlin code
run(Size, C1X, C1Y, C2X, C2Y, MX, MY, DX, DY, HX, HY, Rout) :-
    setCovid(C1X, C1Y),
    setCovid(C2X, C2Y),
    setMask(MX, MY),
    setDoctor(DX, DY),
